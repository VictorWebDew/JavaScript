...
let form = document.forms.age_checker;
form.onsubmit = function (e) {
    e.preventDefault();
    if (+this.customerAge.value < 18) {
        alert('Too young for erotic');
    } else {
        alert('Welcome!');
    }
}
...
menu.onclick = function () {
    this.classList.toggle('toggling');
};
...
for (const comment of comments) {
    ...
    btn.onclick = function() {
        bodyComment.classList.toggle('hideBody');
    }
}
